...
function(cmcs_init_project)
    ...
    foreach(_package IN LISTS ${_VAR_PREFIX}_REQUIRED_PACKAGES ${_VAR_PREFIX}_OPTIONAL_PACKAGES ${_VAR_PREFIX}_OPTIONAL_CONDITIONAL_PACKAGES)
        list(APPEND PACKAGE_SINGLE_ARG "${_package}_VERSION" )
        list(APPEND PACKAGE_MULTI_ARG "${_package}_COMPONENTS;${_package}_FIND_OPTIONS;${_package}_CONDITIONAL" )
    endforeach()
    ...
    foreach(_package IN LISTS ${_VAR_PREFIX}_OPTIONAL_PACKAGES)
        ...
        if(NOT ${${_VAR_PREFIX}_PACKAGE_NAME}_ENABLE_${_package})
            ...
            set(CMAKE_DISABLE_FIND_PACKAGE_${_package} TRUE CACHE BOOL "Disable search for ${_package}")    
        endif()        
    ...
    foreach(_package IN LISTS ${_VAR_PREFIX}_OPTIONAL_CONDITIONAL_PACKAGES)
        ...
        CMAKE_DEPENDENT_OPTION(${${_VAR_PREFIX}_PACKAGE_NAME}_ENABLE_${_package} "Enable usage of package ${_package} within ${PROJECT_NAME}" ${${_VAR_PREFIX}_${_package}_CONDITIONAL} OFF)
        ...
        if(NOT ${${_VAR_PREFIX}_PACKAGE_NAME}_ENABLE_${_package})
            # Trick to make feature summary work with optional dependencies! 
            set(CMAKE_DISABLE_FIND_PACKAGE_${_package} TRUE CACHE INTERNAL "")    
        endif() 
        find_package(${_find_string}) # Cannot have this in an if for FeatureSummary
        if(${${_VAR_PREFIX}_PACKAGE_NAME}_ENABLE_${_package} AND NOT _package)
            cmcs_error_message("${_package} requested but not found!")
